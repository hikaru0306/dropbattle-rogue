# -*- coding: utf-8 -*-
"""ガレス(hero3) 第8弾（g5系を詰める: 兜のライオン紋章・たてがみのシルエット・装飾増） → assets_cand/
...
NEG = ("worst quality, low quality, score_1, score_2, score_3, blurry, jpeg artifacts, sepia, "
    ...
    "beard, chin fur, fur under the chin, fur bib, fur ruff around the neck, "
    "mane hanging down over the chest, mane in front of the face, facial hair, whiskers, "
    "fluffy collar, fur trim collar")
...
GARETH = ("A stalwart anthropomorphic lion heavy knight, "
    ...
    "closed visor with only a narrow horizontal eye slit, pitch dark shadow inside the slit, "
    "no facial features visible at all, "
    "a golden lion head emblem embossed on the brow of the helmet, engraved gold scrollwork on the cheek plates, "
    "spiky golden brown mane tufts flare out from the back and sides of the helmet, "
    "forming a proud lion mane silhouette behind the helmet, "
    "clean bare steel gorget and neck armor, absolutely no fur below the helmet, "
    "no fur on the chin, jaw or chest, "
    "massive heavy steel plate armor in gunmetal and deep blue steel, thick layered pauldrons with "
    "engraved gold filigree, gold trim lining every armor plate edge, chainmail at the joints, "
    "a dark blue tabard with gold embroidered borders and a lion emblem, "
    "wide leather belts with heavy buckles and side pouches, armored greaves and sabatons, "
    "imposing bulky silhouette, "
...
JOBS = {
    "hero3_h1": (STYLE + GARETH + "calm stoic expression, sharp eyes.", 121501),
    "hero3_h2": (STYLE + GARETH + "brave dignified expression.", 121602),
    "hero3_h3": (STYLE + GARETH + "noble commanding expression.", 121703),
    "hero3_h4": (STYLE + GARETH + "confident fierce expression.", 121804),
    "hero3_h5": (STYLE + GARETH + "quiet resolute expression.", 121905),
    "hero3_h6": (STYLE + GARETH + "steady watchful expression.", 122006),
    "hero3_h7": (STYLE + GARETH + "battle ready stance, weight on the back foot.", 122107),
    "hero3_h8": (STYLE + GARETH + "imposing guardian stance.", 122208),
}

def build(seed, text):
    return {"1":{"class_type":"SaveImage","inputs":{"images":["8",0],"filename_prefix":"db_hero3h"}},
