...
ALL="scorp treant hound dryad peng cobra sphinx golem mudman fgolem ifrit alraune skadi slime shade bee fox wisp crab sludge bomber moth owl frost scarab banshee eye voids gazer bat kslime".split()
GROUPS=[(f"f{i//3+1}", ALL[i:i+3]) for i in range(0,len(ALL),3)]
errors=[]
with sync_playwright() as p:
    ...
    for label,kinds in GROUPS:
        ...
        page.screenshot(path=SHOT+rf"\shot_front_{label}.png")
    b.close()
