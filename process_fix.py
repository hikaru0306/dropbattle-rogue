# -*- coding: utf-8 -*-
"""assets_raw_fix/ → 透過・トリム・縮小 → assets/（process_ura.py と同ロジック）
--flip 指定の名前は左右反転してから保存（右向きで出た場合用）
usage: python process_fix.py [name ...] [--flip name,name]"""
import os, sys
import numpy as np
...
SRC = os.path.join(BASE, "assets_raw_fix")
...
args = [a for a in sys.argv[1:] if not a.startswith("--flip")]
flips = set()
for a in sys.argv[1:]:
    if a.startswith("--flip"):
        flips = set(a.split("=",1)[1].split(",")) if "=" in a else set()
targets = [n if n.endswith(".png") else n + ".png" for n in args] or sorted(os.listdir(SRC))
for fn in targets:
    ...
    print(fn, out.size, os.path.getsize(path)//1024, "KB", "FLIPPED" if fn[:-4] in flips else "")
print("DONE")
